    ...
    return (
      <div>
        ...
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="h-9 w-72 animate-pulse rounded-lg bg-surface-hover" />
          <div className="h-9 w-40 animate-pulse rounded-lg bg-surface-hover" />
        </div>
              ...
              <thead className="bg-white/[0.02]">
                <tr>
                  {['Invoice #', 'Customer', 'Items', 'Total', 'Status', 'Date', 'Actions'].map(
                    (h) => (
                      <th key={h} className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-text-secondary">
                        {h}
                      </th>
                    ),
                  )}
  ...
  if (error && sales.length === 0) {
    return (
      <div>
        ...
        <div
          role="alert"
          className="rounded-xl border border-danger/20 bg-danger-light p-8 text-center"
        >
          <p className="text-sm font-medium text-danger">{error}</p>
          <button
            onClick={fetchSales}
            className="mt-4 rounded-lg bg-gradient-to-r from-red-500 to-red-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-red-500/50 focus:ring-offset-2"
          >
            Retry
          </button>
        </div>
    ...
    <div>
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
        <input
          ...
          className="block w-full rounded-lg border border-border bg-surface-card px-4 py-2 text-sm text-text-primary placeholder-slate-500 shadow-sm transition-colors focus:border-border focus:outline-none focus:ring-1 focus:ring-primary/50 sm:w-72"
        />
        <select
          value={statusFilter}
          onChange={handleStatusChange}
          aria-label="Filter by status"
          className="block w-full rounded-lg border border-border bg-surface-card px-4 py-2 text-sm text-text-primary shadow-sm transition-colors focus:border-border focus:outline-none focus:ring-1 focus:ring-primary/50 sm:w-40"
        >
      ...
      {!loading && !error && sales.length === 0 && (
        <div className="rounded-xl border border-dashed border-border bg-surface-card px-6 py-16 text-center shadow-sm">
          ...
          <h3 className="mt-4 text-sm font-semibold text-text-primary">No sales found</h3>
          <p className="mt-1 text-sm text-text-secondary">
                    ...
                    <td className="whitespace-nowrap px-4 py-3 text-right">
                      <div className="flex justify-end gap-1">
                        <Link
                          ...
                          className="rounded-md px-2.5 py-1.5 text-sm font-medium text-primary transition-colors hover:bg-surface-hover hover:text-primary-dark focus:outline-none focus:ring-2 focus:ring-primary/50"
                        >
                          View
                        </Link>
                        {sale.status === 'pending' && (
                          <button
                            ...
                            className="rounded-md px-2.5 py-1.5 text-sm font-medium text-success transition-colors hover:bg-emerald-500/10 hover:text-emerald-300 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 disabled:opacity-50"
                          >
