...
export default function ClientPage() {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <span className="ml-3 text-sm text-text-secondary">Loading branch…</span>
      </div>
    );
  }
  ...
  if (notFound) {
    return (
      <div className="rounded-xl border border-dashed border-border bg-surface-card px-6 py-16 text-center">
        <p className="text-lg font-semibold text-text-primary">Branch not found</p>
        <p className="mt-1 text-sm text-text-secondary">The branch you are looking for does not exist or was deleted.</p>
        <Link href="/dashboard/rh/branches" className="mt-4 inline-block text-sm font-medium text-primary hover:text-primary-dark">
  ...
  if (!form) {
    return (
      <div className="rounded-xl border border-dashed border-border bg-surface-card px-6 py-16 text-center">
        <p className="text-sm text-danger">{submitError || 'Error loading branch data.'}</p>
        <Link href="/dashboard/rh/branches" className="mt-4 inline-block text-sm font-medium text-primary hover:text-primary-dark">
          &larr; Back to branches
        </Link>
      </div>
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/dashboard/rh/branches" className="text-sm font-medium text-primary hover:text-primary-dark">
          &larr; Back
        </Link>
      </div>
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          <div className="flex items-center gap-2">
            <input id="isActive" name="isActive" type="checkbox"
              ...
              className="rounded border-border bg-surface-card text-primary focus:ring-primary/50" />
