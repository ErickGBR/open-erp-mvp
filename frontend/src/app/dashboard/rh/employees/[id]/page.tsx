...
export default function EditEmployeePage() {
    ...
    async function load() {
      try {
        ...
        if (err instanceof Error && err.message.includes('404')) {
          ...
          setSubmitError(err instanceof Error ? err.message : 'Error al cargar empleado');
        }
      } finally {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setSubmitError(err instanceof Error ? err.message : 'Error al actualizar empleado');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setSubmitError(err instanceof Error ? err.message : 'Error al eliminar empleado');
    } finally {
      setDeletingId(null);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <span className="ml-3 text-sm text-slate-400">Cargando empleado…</span>
      </div>
    );
  }
  ...
  if (notFound) {
    return (
      <div className="rounded-xl border border-dashed border-cyan-500/20 bg-[#12121e] px-6 py-16 text-center">
        <p className="text-lg font-semibold text-[#e2e8f0]">Empleado no encontrado</p>
        <p className="mt-1 text-sm text-slate-400">El empleado que busca no existe o ha sido eliminado.</p>
  ...
  if (!form) {
    return (
      <div className="rounded-xl border border-dashed border-cyan-500/20 bg-[#12121e] px-6 py-16 text-center">
        <p className="text-sm text-red-400">{submitError || 'Error al cargar datos del empleado.'}</p>
        <Link href="/dashboard/rh/employees" className="mt-4 inline-block text-sm font-medium text-cyan-400 hover:text-cyan-300">
          &larr; Volver a empleados
        </Link>
      </div>
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#e2e8f0]">Editar Empleado</h1>
      ...
      <form onSubmit={handleSubmit} noValidate className="glass-card rounded-xl p-6 space-y-6">
        {/* Status */}
        <div>
          <h2 className="text-sm font-semibold text-slate-300 mb-3 border-b border-cyan-500/10 pb-2">Estado</h2>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label htmlFor="position" className="block text-sm font-medium text-slate-300">Cargo</label>
        ...
        {/* Documents */}
        <div>
          <h2 className="text-sm font-semibold text-slate-300 mb-3 border-b border-cyan-500/10 pb-2">Documentos</h2>
        ...
        {/* Salary */}
        <div>
          <h2 className="text-sm font-semibold text-slate-300 mb-3 border-b border-cyan-500/10 pb-2">Información Salarial</h2>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
            ...
            <div>
              <label htmlFor="baseSalary" className="block text-sm font-medium text-slate-300">Salario Base <span className="text-red-500">*</span></label>
        ...
        {/* Bank */}
        <div>
          <h2 className="text-sm font-semibold text-slate-300 mb-3 border-b border-cyan-500/10 pb-2">Información Bancaria</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
        ...
        {/* Social Security */}
        <div>
          <h2 className="text-sm font-semibold text-slate-300 mb-3 border-b border-cyan-500/10 pb-2">Seguridad Social</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-cyan-500/10">
          ...
          >
            {deletingId === employeeId ? 'Eliminando…' : 'Eliminar Empleado'}
          </button>
